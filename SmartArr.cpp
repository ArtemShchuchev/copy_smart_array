...
SmartArr::SmartArr(const int capacity) : size_(0), capacity_(capacity > 0 ? capacity : 1)
{
	array_ = new int[capacity_];
}

SmartArr::SmartArr(const SmartArr& sma) : size_(sma.size_), capacity_(sma.capacity_)
{
	array_ = new int[capacity_];
...
void SmartArr::add(const int element)
{
...
int SmartArr::get(const int index)
{
...
// одинаковые данные
SmartArr& SmartArr::operator=(const SmartArr& sma)
{
	if (&sma != this)	// если пытаемся присвоить объект самому себе -> пропустить
	{
		delete[] array_;
		size_ = sma.size_;
